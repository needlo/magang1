<?php
        ...
        // Menentukan Posisi/Role
        $posisi = 'User';
        if ($currentUser) {
            if ($currentUser['role'] == 'intern') {
                $intern = $db->table('internship_applications')->where('user_id', $session_id)->get()->getRowArray();
                $posisi = $intern['minat_posisi'] ?? 'Peserta Magang';
            } else {
                // Jika Admin atau Mentor, langsung kapitalisasi nama role-nya
                $posisi = ucfirst($currentUser['role']); 
            }
        }

        // Memecah nama untuk nama panggilan (di sebelah foto kecil)
        $namaLengkap = $currentUser['name'] ?? 'User';
                ...
                <ul class="flex items-center justify-center md:justify-normal mt-[13px] md:mt-0">
                    ...
                    <li class="relative profile-menu mx-[8px] md:mx-[10px] lg:mx-[12px] ltr:first:ml-0 ltr:last:mr-0 rtl:first:mr-0 rtl:last:ml-0">
                        <button type="button" class="flex items-center -mx-[5px] relative ltr:pr-[14px] rtl:pl-[14px] text-black dark:text-white" id="dropdownToggleBtn">
                            ...
                            <img src="<?= $fotoProfil ?>" class="w-[35px] h-[35px] md:w-[42px] md:h-[42px] rounded-full ltr:md:mr-[2px] ltr:lg:mr-[8px] rtl:md:ml-[2px] rtl:lg:ml-[8px] border-[2px] border-primary-200 inline-block object-cover" alt="profile-image">
                        ...
                        <div class="profile-menu-dropdown bg-white dark:bg-[#0c1427] transition-all shadow-3xl dark:shadow-none py-[22px] absolute mt-[13px] md:mt-[14px] w-[195px] z-[1] top-full ltr:right-0 rtl:left-0 rounded-md">
                            <div class="flex items-center border-b border-gray-100 dark:border-[#172036] pb-[12px] mx-[20px] mb-[10px]">
                                ...
                                <img src="<?= $fotoProfil ?>" class="rounded-full w-[31px] h-[31px] ltr:mr-[9px] rtl:ml-[9px] border-2 border-primary-200 inline-block object-cover" alt="profile-image">
                                
                                <div>
                                    <span class="block text-black dark:text-white font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[110px]">
                                        <?= esc($namaLengkap) ?>
                                    </span>
                            ...
                            <ul>
                                <li>
                                    <a href="<?= base_url('profile') ?>" class="block relative py-[7px] ltr:pl-[50px] ltr:pr-[20px] rtl:pr-[50px] rtl:pl-[20px] text-black dark:text-white transition-all hover:text-primary-500">
                            ...
                            <ul>
                                ...
                                <li>
                                    <a href="<?= base_url('logout') ?>" class="block relative py-[7px] ltr:pl-[50px] ltr:pr-[20px] rtl:pr-[50px] rtl:pl-[20px] text-black dark:text-white transition-all hover:text-danger-500">
                                        <i class="material-symbols-outlined top-1/2 -translate-y-1/2 !text-[22px] absolute ltr:left-[20px] rtl:right-[20px]">
                                            logout
                                        </i>
                                        Logout
